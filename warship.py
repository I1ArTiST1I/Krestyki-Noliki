...
class Field(object):
    ...
    def __init__(self, size):
        self.size = size
        self.map = [
            [Cell.empty_cell for _ in range(size)] for _ in range(size)]
        self.radar = [
            [Cell.empty_cell for _ in range(size)] for _ in range(size)]
    ...
    def get_input(self, input_type):
        ...
        if input_type == "ship_setup":
            ...
            if self.is_ai or self.auto_ship_setup:
                user_input = str(
                    choice(Game.letters)) + str(randrange(0, self.field.size)) + choice(["H", "V"])
            else:
        ...
        if input_type == "shot":
            ...
            if self.is_ai:
                ...
                if self.skill == 0:
                    x, y = randrange(0, self.field.size), randrange(
                        0, self.field.size)
            else:
                user_input = input().upper().replace(" ", "")
                ...
                if x not in Game.letters or not y.isdigit() or int(y) not in range(1, Game.field_size + 1):
                    self.message.append(
                        'Приказ непонятен, ошибка формата данных')
                    return 500, 0
...
if __name__ == '__main__':
    ...
    players = []
    players.append(
        Player(name='I_ArTiST_I', is_ai=False, auto_ship=True, skill=1))
    players.append(Player(name='I_ArTiST_I_AI',
                   is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'game over':
            ...
            print('{} выиграл матч! Поздравления!'.format(
